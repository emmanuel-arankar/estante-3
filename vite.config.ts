  ...
  return {
    ...
    build: {
      rollupOptions: {
        output: {
          manualChunks(id) {
            // Bolt: Use function-based strategy to avoid errors with externalized modules in SSR
            if (id.includes('node_modules')) {
              if (id.includes('react/') || id === 'react' || id.includes('react-dom/') || id === 'react-dom' || id.includes('react-router-dom/')) {
                return 'react-vendor';
              }
