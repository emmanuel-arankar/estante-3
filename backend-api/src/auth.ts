...
router.post('/register', authLimiter as unknown as RequestHandler, validate({ body: registerSchema }), async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const fbAuthError = authError as FirebaseError;
      console.error('CRITICAL: authError dump ->', fbAuthError);
      // ==== ==== 2. TRATAMENTO DE COLISÃO DE E-MAIL ==== ====
      if (fbAuthError?.code === 'auth/email-already-exists') {
        return res.status(400).json({ error: 'E-mail já está em uso.' });
      }
      return res.status(500).json({ error: 'Erro ao criar conta no Firebase.', details: fbAuthError?.message || JSON.stringify(fbAuthError) || String(fbAuthError) });
    }
    ...
    try {
      ...
      const fbDbError = dbError as FirebaseError;
      logger.error('CRITICAL: Erro oculto ao salvar perfil no DB:', fbDbError);
      await admin.auth().deleteUser(uid).catch(() => logger.error(`Falha no rollback do user ${uid}`));
      return res.status(500).json({ error: 'Erro ao configurar perfil de usuário. Tente novamente.', details: fbDbError?.message || fbDbError });
    }
