...
router.post('/sessionLogin', authLimiter as unknown as RequestHandler, validate({ body: sessionLoginBodySchema }), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const firebaseError = error as FirebaseError & { message: string; code: string };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errData?.error?.message) {
        const fbError = errData.error.message;
      ...
      throw new Error(errData.error?.message || 'Erro na autenticação.');
    }
...
router.post('/recover', authLimiter as unknown as RequestHandler, validate({ body: recoverSchema }), async (req: Request, res: Response) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errData.error?.message || 'Erro ao enviar email de recuperação.');
    }
