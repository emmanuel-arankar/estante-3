 ...
 */
router.post('/sessionLogin', authLimiter as unknown as RequestHandler, validate({ body: sessionLoginBodySchema }), async (req, res) => {
  ...
  try {
    ...
    const firebaseError = (error as any) as FirebaseError;
 ...
 */
router.post('/register', authLimiter as express.RequestHandler, validate({ body: registerSchema }), async (req: Request, res: Response) => {
  try {
    // A validação agora é feita pelo middleware 'validate'
 ...
 */
router.post('/login', authLimiter as express.RequestHandler, validate({ body: loginSchema }), async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/recover', authLimiter as express.RequestHandler, validate({ body: recoverSchema }), async (req: Request, res: Response) => {
  try {
    const apiKey = getFirebaseApiKey();
    if (!apiKey) {
      return res.status(500).json({ error: 'Configuração do servidor ausente (FIREBASE_API_KEY).' });
