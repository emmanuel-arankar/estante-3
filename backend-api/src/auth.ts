...
router.post('/register', authLimiter as any, validate({ body: registerSchema }), async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      return res.status(500).json({ error: 'Erro ao criar conta no Firebase.', details: error?.message || JSON.stringify(authError) || String(authError) });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: 'Erro ao configurar perfil de usuário. Tente novamente.', details: error?.message || dbError });
    }
...
router.post('/login', authLimiter as any, validate({ body: loginSchema }), async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Erro no login do backend:', err.message || error);
...
router.post('/recover', authLimiter as any, validate({ body: recoverSchema }), async (req: Request, res: Response) => {
  try {
    ...
    const err = error as { message?: string };
    logger.error('Erro na recuperação de senha:', err.message || error);
...
router.post('/google', async (req: Request, res: Response) => {
  try {
    ...
    const err = error as { message?: string };
    logger.error('Erro login google backend:', err.message || error);
