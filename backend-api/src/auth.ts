...
router.post('/register', authLimiter as RequestHandler, validate({ body: registerSchema }), async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      logger.error('CRITICAL: Erro oculto ao salvar perfil no DB:', dbErr);
      await admin.auth().deleteUser(uid).catch(() => logger.error(`Falha no rollback do user ${uid}`));
      return res.status(500).json({ error: 'Erro ao configurar perfil de usuário. Tente novamente.', details: dbErr?.message || String(dbErr) });
    }
...
router.post('/login', authLimiter as RequestHandler, validate({ body: loginSchema }), async (req: Request, res: Response) => {
  try {
    ...
    const successData = data as { localId: string };
    const { localId } = successData;
    ...
    return res.status(200).json({ customToken });
      ...
      const err = error as Error;
      logger.error('Erro no login do backend:', err.message || String(err));
...
router.post('/recover', authLimiter as RequestHandler, validate({ body: recoverSchema }), async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({ message: 'E-mail enviado' });
      ...
      const err = error as Error;
      logger.error('Erro na recuperação de senha:', err.message || String(err));
...
router.post('/google', async (req: Request, res: Response) => {
  try {
    ...
    const err = error as Error;
    logger.error('Erro login google backend:', err.message || String(err));
