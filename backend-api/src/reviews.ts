...
import { checkAuth, checkAuthOptional, AuthenticatedRequest } from './middleware/auth.middleware';
import {
    createReviewSchema,
    updateReviewSchema,
    reviewIdParamSchema,
...
const sanitizeTimestamps = (data: Record<string, unknown>) => {
    ...
    for (const key of ['createdAt', 'updatedAt']) {
        const val = result[key] as { toDate?: () => Date } | undefined;
        if (val?.toDate) result[key] = val.toDate();
    }
    return result;
};
...
async function recalculateEditionMetrics(editionId: string) {
    ...
    snapshot.docs.forEach(doc => {
        const data = doc.data() as { title?: string; content?: string; rating?: number };
...
router.post('/reviews', checkAuth, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        let workId = typeof editionData.workId === 'string' ? editionData.workId : (editionData.workId as { id: string })?.id;
        if (!workId) {
        ...
        if (!existingReviewQuery.empty) {
            ...
            const reviewData = reviewDoc.data() as { rating: number; editionId: string };
            const updates: Record<string, unknown> = {
                rating: data.rating !== undefined ? data.rating : reviewData.rating,
        ...
        res.status(201).json(sanitizeTimestamps({ id: docRef.id, ...reviewData } as unknown as Record<string, unknown>));
    } catch (error) {
        next(error);
    }
});
 ...
 */
router.get('/reviews/edition/:editionId/my', checkAuth, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const snapshot = await db.collection('reviews')
            ...
            .where('userId', '==', (req as AuthenticatedRequest).user.uid)
            .limit(1)
            .get();
        ...
        const review = sanitizeTimestamps({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as Record<string, unknown>) as Record<string, unknown>;

        // Embora seja a review do próprio usuário, verificamos isLiked por consistência
        const likeDoc = await db.collection('reviews').doc(review.id).collection('likes').doc((req as AuthenticatedRequest).user.uid).get();
...
router.get('/reviews/edition/:editionId', checkAuthOptional, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const userId = (req as Partial<AuthenticatedRequest>).user?.uid;
...
router.get('/reviews/:reviewId', checkAuthOptional, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if ((req as Partial<AuthenticatedRequest>).user?.uid) {
            const likeDoc = await db.collection('reviews').doc(review.id).collection('likes').doc((req as AuthenticatedRequest).user.uid).get();
            review.isLiked = likeDoc.exists;
        }
