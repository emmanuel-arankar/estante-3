 ...
 */
export const rtdb = (process.env.NODE_ENV === 'test' && !process.env.FIREBASE_DATABASE_URL)
  ? { ref: () => ({ set: () => Promise.resolve(), update: () => Promise.resolve(), push: () => ({ key: 'mock', set: () => Promise.resolve() }), get: () => Promise.resolve({ exists: () => false, val: () => null }), transaction: () => Promise.resolve({ committed: true }) }) } as any
  : admin.database();
