...
if (admin.apps.length === 0) {
  ...
  if (fs.existsSync(saPath) && (!isManagedCloud || isEmulator)) {
    try {
      ...
      const credential = admin.credential.cert(saData);
    ...
    // ⚡ BOLT FIX: Ensure Database URL is present in test environment to avoid admin.database() failure
    const projectId = process.env.VITE_FIREBASE_PROJECT_ID || 'estante-75463';
    admin.initializeApp({
      projectId,
      databaseURL: `https://${projectId}-default-rtdb.firebaseio.com`
    });
    logger.info('Firebase Admin inicializado para testes.');
  } else {
