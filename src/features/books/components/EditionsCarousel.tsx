...
export function EditionsCarousel({ workId, currentEditionId }: EditionsCarouselProps) {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => getWorkEditionsFilteredAPI(workId, { limit: 12, page: 1 }),
        enabled: !!workId,
        staleTime: 1000 * 60 * 5,
    });
                            ...
                            {editions.map((edition: Edition) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        title={edition.title}
                                    >
                                        {/* Capa com Interação Premium */}
                                        <div className="w-[165px] h-[248px] rounded-[4px] overflow-hidden bg-gray-50 border border-gray-100 shadow-sm transition-all duration-300 group-hover/item:shadow-xl group-hover/item:-translate-y-1 group-hover/item:scale-[1.02]">
                                            {edition.coverUrl ? (
                                                <img
                                        ...
                                        {/* Metadados Refinados */}
                                        <div className="flex flex-col text-left text-sm leading-tight transition-colors group-hover/item:text-indigo-900">
                                            <p className="text-gray-900 font-semibold line-clamp-1 mb-0.5">
                                                {format?.name || 'Vários'}
                                            </p>
                                            <p className="text-gray-500 line-clamp-1 text-[13px]">
                                                {edition.publisher?.name || 'Desconhecida'}
                                            </p>
                                            <div className="flex items-center gap-1.5 text-gray-400 mt-1 text-[12px] font-medium">
                                                <span>{year || 'n/d'}</span>
