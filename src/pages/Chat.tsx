...
export const Chat = () => {
  ...
  const setReplyingTo = useCallback((message: ChatMessage | null) => {
    if (message) setEditingMessageRaw(null);
    setReplyingToRaw(message);
  }, [setReplyingToRaw, setEditingMessageRaw]);

  const setEditingMessage = useCallback((message: ChatMessage | null) => {
    if (message) setReplyingToRaw(null);
    setEditingMessageRaw(message);
  ...
  // ⚡ BOLT OPTIMIZATION: Single-pass O(N) grouping assuming chronological order
