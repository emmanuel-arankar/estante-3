...
export const Chat = () => {
  ...
  const {
    ...
    isRecording,
    replyingTo,
    setReplyingTo: setReplyingToRaw,
    editingMessage,
    setEditingMessage: setEditingMessageRaw,
    sendMessage,
    deleteMessage,
    deleteChat,
    editMessage,
  ...
  // ⚡ BOLT OPTIMIZATION: Memoized handlers to maintain referential stability for ChatBubble.
  ...
  const setReplyingTo = useCallback((msg: ChatMessage | null) => {
    setEditingMessageRaw(null);
    setReplyingToRaw(msg);
  }, [setEditingMessageRaw, setReplyingToRaw]);

  const setEditingMessage = useCallback((msg: ChatMessage | null) => {
    setReplyingToRaw(null);
    setEditingMessageRaw(msg);
  }, [setReplyingToRaw, setEditingMessageRaw]);
  ...
  // ⚡ BOLT OPTIMIZATION: Optimized grouping logic to O(N) from O(N*G).
  // Leverages the fact that messages are already sorted chronologically.
