...
export const Chat = () => {
  ...
  const {
    messages,
    loading,
    setReplyingTo: setReplyingToInternal,
    setEditingMessage: setEditingMessageInternal,
    isTyping,
    isRecording,
    replyingTo,
    editingMessage,
  ...
  }, [messages, isAtBottom, scrollToBottom, user?.uid]);
  ...
  const setReplyingTo = useCallback((msg: ChatMessage | null) => {
    setEditingMessageInternal(null);
    setReplyingToInternal(msg);
  }, [setEditingMessageInternal, setReplyingToInternal]);

  const setEditingMessage = useCallback((msg: ChatMessage | null) => {
    setReplyingToInternal(null);
    setEditingMessageInternal(msg);
  }, [setReplyingToInternal, setEditingMessageInternal]);

  // Use a ref for messages to keep callbacks stable
  const messagesRefForCallbacks = useRef(messages);
  useEffect(() => {
    messagesRefForCallbacks.current = messages;
  }, [messages]);

  const handlePlayNext = useCallback((currentMessageId: string) => {
    const currentMessages = messagesRefForCallbacks.current;
  ...
  const handleSearchNext = useCallback(() => {
    ...
    setCurrentSearchIndex(prev => {
      const nextIndex = (prev + 1) % searchMatches.length;
  ...
  const handleSearchPrev = useCallback(() => {
    if (searchMatches.length === 0) return;
    setCurrentSearchIndex(prev => {
      const prevIndex = (prev - 1 + searchMatches.length) % searchMatches.length;
  ...
  // ⚡ BOLT OPTIMIZATION: Reduces grouping overhead from O(N*G) to O(N) by leveraging sorted order
      ...
      <main className="h-[calc(100vh-80px)] w-full max-w-6xl mx-auto px-4 pt-8 pb-8 flex flex-col">
        <div className="bg-white shadow-sm flex-1 flex flex-col w-full overflow-hidden rounded-2xl border border-gray-200/60">
          ...
          <div className="border-t border-gray-200 p-4 bg-white shrink-0">
            <ChatInput
              onSendMessage={handleSendMessage as ChatInputProps['onSendMessage']}
