...
export const Chat = () => {
  ...
  // ⚡ BOLT OPTIMIZATION: Use a ref for messages to keep callbacks stable
  ...
  // Stable handlers for ChatBubble to prevent re-renders
  ...
  const handleMarkMessageAsViewed = useCallback((messageId: string) => {
  ...
  // Avoids O(N*G) complexity of .find().
  const groupedMessages = useMemo(() => {
    const groups: { date: Date; messages: ChatMessage[] }[] = [];
    messages.forEach((msg) => {
      // Ensure we have a Date object. Although the service usually provides it,
      // we wrap it to ensure runtime stability with isSameDay.
