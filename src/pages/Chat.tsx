...
export const Chat = () => {
  ...
  // Advanced Stability Pattern: Use a ref to keep track of messages for stable callbacks
  ...
  }, [messages, scrollToBottom, isAtBottom, user?.uid]);
  ...
  const handleOnReply = useCallback((msg: ChatMessage) => {
    setEditingMessage(null);
    setReplyingTo(msg);
  }, [setEditingMessage, setReplyingTo]);

  const handleOnEdit = useCallback((msg: ChatMessage) => {
    setReplyingTo(null);
  ...
  }, [searchQuery, searchMatches.length, searchMatches, scrollToMessage]);

  // Agrupa mensagens por data (não mais filtrado por busca)
  // ⚡ BOLT OPTIMIZATION: Reduz complexidade de O(N*G) para O(N) usando processamento em uma única passada
  // Aproveita o fato de que as mensagens já vêm ordenadas cronologicamente do hook useChat.
