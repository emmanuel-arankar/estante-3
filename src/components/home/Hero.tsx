...
export const Hero = () => {
  ...
  // PERFORMANCE: Memoize background bubble coordinates, durations, and delays
  // using deterministic pseudo-random mathematical values based on the bubble index.
  // This avoids Math.random() in the render loop, preventing visual jitter,
  // reducing re-render performance costs on keystrokes, and avoiding SSR hydration mismatches.
  const bubbles = useMemo(() => {
    return Array.from({ length: 20 }, (_, i) => {
      // Deterministic pseudo-random generation based on index to ensure stable SSR and no layout shifting
      const left = ((i * 37) % 100);
      const top = ((i * 53) % 100);
      const duration = 3 + ((i * 7) % 3); // between 3 and 5s
      const delay = (i * 13) % 4 / 2; // between 0 and 1.5s
