...
export const Hero = () => {
  ...
  // PERFORMANCE: Memoize background bubble coordinates and animation parameters
  // to avoid recalculation, eliminate visual jitter, and ensure deterministic SSR compatibility on re-renders.
  const bubbles = useMemo(() => {
    return Array.from({ length: 20 }, (_, i) => {
      // Deterministic pseudorandom values based on the index to mimic organic distribution
      const leftSeed = (i * 17) % 100;
      const topSeed = (i * 23) % 100;
      const durationSeed = (i * 13) % 3; // generates 0, 1, or 2
      const delaySeed = (i * 7) % 3; // generates 0, 1, or 2
      return {
        left: `${leftSeed}%`,
        top: `${topSeed}%`,
        duration: 3 + durationSeed,
        delay: delaySeed,
      };
    });
  }, []);
